...
Epoll::Epoll() : epfd_(epoll_create1(EPOLL_CLOEXEC)) {
    if (epfd_ < 0) throw_error("epoll_create1");
}
...
void Epoll::add(int fd, uint32_t events, void* ptr) {
    ...
    ev.events = events;
    ev.data.ptr = ptr;
    if (epoll_ctl(epfd_, EPOLL_CTL_ADD, fd, &ev) < 0) throw_error("epoll_ctl ADD");
}
...
void Epoll::mod(int fd, uint32_t events, void* ptr) {
    ...
    ev.events = events;
    ev.data.ptr = ptr;
    if (epoll_ctl(epfd_, EPOLL_CTL_MOD, fd, &ev) < 0) throw_error("epoll_ctl MOD");
}

void Epoll::del(int fd) {
    if (epoll_ctl(epfd_, EPOLL_CTL_DEL, fd, nullptr) < 0) throw_error("epoll_ctl DEL");
}
...
std::vector<struct epoll_event> Epoll::wait(int max_events, int timeout_ms) {
    ...
    if (nfds < 0) throw_error("epoll_wait");
    evs.resize(nfds);
    return evs;
}
...
void Client::connect(const std::string& host, uint16_t port) {
    ...
    if (rc < 0 && errno != EINPROGRESS) throw_error("connect");
